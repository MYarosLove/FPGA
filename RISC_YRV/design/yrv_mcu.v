...
`define INTEL_VERSION						// Мы работаем на интеловских чипах, поэтому не трогай
`define BOOT_FROM_AUX_UART					// Указывает, что мы будем загружать прошивку через UART
													// используется PIN - 34. При подключений переходника
													// USB->UART покдлючай землю (GND) в первую очередь
													// PIN 34 подключается к TX переходника
// Если указано INSTANCE_MEM, тогда микропрограмма code_demo.mem32 из RISC_YRV\prog
// зашивается в память ПЛИС на этапе синтеза
...
module yrv_mcu (
                 ...
                 `ifdef BOOT_FROM_AUX_UART, aux_uart_rx
                 `endif
                 `ifdef EXPOSE_MEM_BUS, mem_ready, mem_rdata, mem_lock, mem_write, mem_trans, mem_ble,
...
`endif
  ...
  .csr_write(),
                    ...
                    .mem_ble(top_mem_ble),
						  .mem_lock(),
						  .mem_trans(top_mem_trans),
                    .mem_wdata(top_mem_wdata), 
						  .mem_write(top_mem_write), 
						  .timer_en(),
                    .wfi_state(wfi_state), 
						  .brk_req(1'b0), 
						  .bus_32(bus_32), 
